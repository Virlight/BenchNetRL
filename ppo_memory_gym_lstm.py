...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_branches = nn.ModuleList(
            [
                layer_init(nn.Linear(256, out_features=num_actions), np.sqrt(0.01))
                for num_actions in [3, 3]
            ]
        )
    ...
    def get_states(self, x, lstm_state, done):
        ...
        for h, d in zip(hidden, done):
            ...
            new_hidden += [h]
        new_hidden = torch.cat(new_hidden, dim=0).reshape(-1, 256)
    ...
    def get_action_and_value(self, x, lstm_state, done, action=None):
        ...
        logits_list = [branch(hidden) for branch in self.actor_branches]
        dists = [Categorical(logits=logits) for logits in logits_list]

        if action is None:
            actions_dim = [dist.sample() for dist in dists]
            action_2d = torch.stack(actions_dim, dim=1)
        else:
            action_2d = action
            actions_dim = [action_2d[:, i] for i in range(action_2d.shape[1])]
        
        log_probs_dim = [dist.log_prob(a_dim) for dist, a_dim in zip(dists, actions_dim)]
        logprob = torch.stack(log_probs_dim, dim=1).sum(dim=1)  # shape (batch_size,)
        
        entropies_dim = [dist.entropy() for dist in dists]
        entropy = torch.stack(entropies_dim, dim=1).sum(dim=1)  # shape (batch_size,)
        
        value = self.critic(hidden).flatten()
        return action_2d, logprob, entropy, value, lstm_state
...
if __name__ == "__main__":
    ...
    print(envs.single_action_space.shape) # (2,)
    print(envs.single_action_space) # MultiDiscrete([3 3])
    print(envs.single_observation_space.shape) # (84, 84, 3)
    print(envs.single_observation_space) # Box(0, 255, (84, 84, 3), uint8)
